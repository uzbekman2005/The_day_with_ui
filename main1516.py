...
class FifWindow(QMainWindow):
    def __init__(self,  user_id, parent=None):
        super(FifWindow, self).__init__(parent)
        self.ui = Ui_Main1516()
        self.userID = user_id
    ...
    def warningMsg(self):
        ...
        if res == QMessageBox.StandardButton.Yes:
            ...
            self.addScoreToDatabase(self.get_userScore() + score)
            while True:
                sleep(3)
                break
            self.makeTheCubicRand()
    ...
    def get_userScore(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = f"SELECT * from users where user_id = {self.userID}"
                cursor.execute(sql)
                return cursor.fetchall()[0]["score"]
        except Exception as ex:
            print(ex)



    def addScoreToDatabase(self, score):
        self.db_connection()
        try:
            print('Update')
            with self.connection.cursor() as cursor:
                sql = f"update users set score = %s where user_id = {self.userID}"
